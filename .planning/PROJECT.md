...
- [x] Source-agnostic input, but **intelligently prioritized**: when multiple source subtitles exist for the same media, prefer the source language whose honorific/relational system best preserves the information Vietnamese needs (prefer a Chinese/Korean/Japanese/Thai source for East-Asian content over English); fall back to whatever is available — Validated in Phase 10 (SRC-01, SRC-02). `original_language` captured from Sonarr/Radarr into `arr_metadata`; `rank_sources` ranks the full available-language set by a relational-richness tier biased so the **native original-language source always wins** (a native Spanish source is never displaced by a Korean fansub), with English the universal fallback; the chosen source is wired through `process_one_item` → `select_source_for_item` → `translate_file` (per-series override + Bazarr inventory honored). The D-110 richer-source upgrade re-translates without clobbering a foreign `.vi` sidecar (D-26/27/28 preserved).
...
| Source-agnostic input, prioritized by relational fidelity to Vietnamese | English flattens relationships; the original East-Asian source preserves the info Vietnamese needs | ✓ Implemented (Phase 10, SRC-01/02 + INTG-02) — Bazarr inventory (soft dependency) + `original_language`-biased relational-richness ranking; native-original-language source wins; per-series overrides (SVC-05) for source/register/model |
